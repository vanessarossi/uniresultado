...
function pesquisarPagina(numeroPagina) {
	...
	$.ajax({
		url : '/uniresultado/resultado/pesquisa/migra',
		type : 'get',
		data: {page : numeroPagina},
		beforeSend : function(){}
	})
	.done(function(response){
		var listaMigra = response["content"];
		totalPaginas = response["totalPages"];
		totalElementos = response["totalElements"];
		numero = response["number"];
		montarTabela(listaMigra);
...
function montarTabela(listaMigra) {
	$('#tabelaMigra > tbody > tr').remove();
	for (var i = 0; i < listaMigra.length; i++) {
		
		var row = "<tr>";
		    row += "<td>"+ listaMigra[i]["id"] +"</td>";
		    row += "<td>"+ listaMigra[i]["accessionNumber"]+"</td>";
		    row += "<td>"+ '<a href="/uniresultado/resultado/excluir/migra/'+listaMigra[i]["id"]+'" class="btn btn-sm btn-danger"> <i class="fas fa-trash-alt"></i> </a>'+ "</td>";
			row += "</tr>";

		$('#tabelaMigra').append(row);
	}
}
